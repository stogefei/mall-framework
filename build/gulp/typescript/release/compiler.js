...
class ProjectCompiler {
  ...
  emit (result, preEmitDiagnostics, callback) {
    ...
    for (const error of diagnostics) {
       ...
       * 2339 参数未找到
       */
      if (![2307, 2339].includes(error.code)) {
  ...
  emitFile ({
    file, jsFileName, dtsFileName, dtsMapFileName, jsContent, dtsContent, dtsMapContent, jsMapContent,
  }, currentDirectory) {
    let base;
...
class FileCompiler {
  ...
  inputFile (file) {
    ...
    const map = JSON.parse(Buffer.from(mapString, 'base64').toString());
